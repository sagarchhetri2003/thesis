...
st.set_page_config(page_title="Liverpool FC Analysis", layout="wide")
...
st.markdown(
    ...
    <style>
        ...
        .main {
            background-image: url('https://upload.wikimedia.org/wikipedia/commons/thumb/a/a3/Anfield_Stadium_2012.jpg/1200px-Anfield_Stadium_2012.jpg');
...
st.image("https://upload.wikimedia.org/wikipedia/en/0/0c/Liverpool_FC.svg", width=100)
st.title("🔴 Liverpool FC Full Analysis Dashboard")

# === Sidebar ===
st.sidebar.image("https://upload.wikimedia.org/wikipedia/en/0/0c/Liverpool_FC.svg", width=80)
...
with tabs[0]:
    st.markdown("### 🔴 Liverpool Overview")
...
# Define df_temp before sorting
df_temp = pd.DataFrame({
    'Team': ['Liverpool', 'Manchester City', 'Chelsea', 'Arsenal'],
    'xG_h': [2.5, 2.1, 1.8, 1.6]
})

# Sort df_temp by xG_h in descending order
...
fig12.update_traces(textposition='top center', marker=dict(size=10, color='red'))
fig12.update_layout(
    title_font=dict(size=22),
...
#     lambda row: "Home" if row["h_team"] == "Liverpool" else "Away",
#     axis=1
...
#     ax.set_title(f"Liverpool Shot Map ({title})")
#     ax.set_xlim(0.7, 1.05)
#     ax.set_ylim(0.9, 0.2)  # ✅ Flip Y axis correctly for football pitch view
#     ax.set_aspect('equal')
...
# liverpool_shots = df[df["team"] == "Liverpool"]  # or filter by any relevant condition
# liverpool_shots["isGoal"] = liverpool_shots["result"] == "Goal"
...
with tabs[3]:
    st.markdown("### ⚖️ Match Situations & Outcomes")


with tabs[4]:
    st.markdown("### 🧠 Managerial Stats")
...
with tabs[6]:
    st.markdown("### 🧱 League-wide Metrics")
st.pyplot(plt)
...
# team_name = "Liverpool"
# # Load the match results data into df_copy
# df_copy = pd.read_csv("EPL_result.csv") 
